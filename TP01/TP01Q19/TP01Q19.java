...
public class TP01Q19 {
  ...
  public static int primeiro(String palavra, int i) {
    for (int x = i; x < palavra.length(); x++) {
      if (palavra.charAt(x) >= '0' && palavra.charAt(x) <= '9') return x;
    }
    return -1;
  }
      ...
      while (linha.length() > 1) {
        ...
        if (!expressao.startsWith("not")) {
          ...
          for (int x = 0; x < c; x++) {
            ...
            parametros[x] = Integer.parseInt(expressao.substring(p, p + 1));
            p++;
          }
          ...
          if (expressao.equals("not(0)")) {
            linha = linha.replace("not(0)", "1");
          } else if (expressao.equals("not(1)")) {
