...
public class Shows implements Cloneable {
  ...
  public static void main(String[] args) throws IOException {
    ...
    PrintWriter log = new PrintWriter(
      new FileWriter(matricula + "_insercao.txt")
    );
  ...
  /**
   * Ordena a lista de Shows usando o algoritmo de inserção.
  ...
  public static int[] ordenarHeap(List<Shows> lista) {
    ...
    for (int i = 1; i < lista.size(); i++) {
      ...
      // Comparações por type e desempate por title
      while (j >= 0) {
        comparacoes++;
        int cmpType = lista.get(j).getType().compareTo(temp.getType());

        if (
          cmpType > 0 ||
          (
            cmpType == 0 &&
            lista.get(j).getTitle().compareTo(temp.getTitle()) > 0
          )
        ) {
