...
/**
 * Estrutura para armazenar informações de uma produção (Show).
 */
typedef struct
{
    char show_id[20];
...
/**
 * Remove caracteres de nova linha de uma string.
 */
...
/**
 * Limpa campos com aspas e caracteres duplicados.
 */
...
/**
 * Imprime os dados de uma produção no formato solicitado.
 */
...
/**
 * Procura uma produção por ID no CSV e preenche a estrutura Show.
 */
...
/**
 * Compara dois Shows com base em tipo e título.
 */
int compararShows(Show *a, Show *b)
{
    comparacoes++;
    int type_cmp = strcmp(a->type, b->type);
    return type_cmp != 0 ? type_cmp : strcmp(a->title, b->title);
}

/**
 * Ordena os Shows usando Shell Sort.
 */
...
/**
 * Gera o log com tempo de execução e número de comparações.
 */
void escreverLog(double tempo)
{
    FILE *log = fopen("771703_shellsort.txt", "w");
    if (log)
    {
